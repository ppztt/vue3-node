...
const handleEdit = async (data) => {
  const res = await axios.get(`/admin/user/list/${data._id}`);
...
const handleEditConfirm = () => {
  userFormRef.value.validate(async (valid) => {
    if (valid) {
      ...
      //1- 更新后端
      let res = await axios.put(`/admin/user/list/${userForm._id}`, userForm);
...
const handleDelete = async (data) => {
  let res = await axios.delete(`/admin/user/list/${data._id}`);
  if (res.data.code === 200) {
